...
acpi
----
Advanced Configuration and Power Interface daemon. ACPI events, such as closing
laptop lid or pressing the power button, may be handled by this daemon. Handler
scripts are in /etc/acpi. It is possible that these handlers may conflict with
other mechanisms that also handle the same event. Perhaps there is not much in
/etc/acpi anyway. If you don't see any specific use for this, turn it off.

cups
----
This runs the CUPS daemon 'cupsd', required for printing. Leave this enabled unless
...
applications use this. An example is 'gecko-mediaplayer' (browser plugin) that
uses DBUS to communicate with 'gnome-mplayer' (multimedia player). So, unless
you know that no apps require this, leave it enabled.
