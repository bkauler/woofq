...
#150116 log all missing firmware to /root/.network-firmware-missing
#151101 fix.
#20220319 handle wildcards, ex: aFW=brcm/brcmfmac*-sdio.*.txt
...
echo -n "" > /tmp/networkfirmware-all
echo -n "" > /tmp/networkfirmware-missing
echo "$RAWLIST" |
while read ONERAW
do
 ...
 if [ "$ONEFW" ];then
  echo "$ONEFW" >> /tmp/networkfirmware-all
  for aFW in `echo "$ONEFW" | tr '\n' ' '`
  do
   #20220319 handle wildcards, ex: aFW=brcm/brcmfmac*-sdio.*.txt
   # see also /usr/local/firmware/extract-fw-all
   if [ "${aFW/\*/}" != "${aFW}" ];then
    ls -1 /lib/firmware/${aFW} >/dev/null 2>&1
    if [ $? -ne 0 ];then
     #not satisfactory, as only one hit will avoid this...
     echo "$aFW" >> /tmp/networkfirmware-missing
    fi
   else
    if [ ! -f /lib/firmware/${aFW} ];then
     echo "$aFW" >> /tmp/networkfirmware-missing #151101
    fi
   fi
  done
 fi
...
sort -u /tmp/networkfirmware-all > /root/.network-firmware-all #150116
sort -u /tmp/networkfirmware-missing > /root/.network-firmware-missing #150116
