...
if [ ! -e usr/lib/libgphoto2.so.2 ];then
 ...
 if [ "$FNDLIBGPHOT" ];then
  ...
  ln -s $BASELIBGPHOT usr/lib/libgphoto2.so.2 2>/dev/null
  ln -s $BASELIBGPHOT usr/lib/libgphoto2.so.6 2>/dev/null
 fi
fi
if [ ! -e usr/lib/libgphoto2_port.so.0 ];then
 ...
 if [ "$FNDLIBGPHOT" ];then
  BASELIBGPHOT="$(basename $FNDLIBGPHOT)"
  ln -s $BASELIBGPHOT usr/lib/libgphoto2_port.so.0 2>/dev/null
  ln -s $BASELIBGPHOT usr/lib/libgphoto2_port.so.12 2>/dev/null
 fi
fi
