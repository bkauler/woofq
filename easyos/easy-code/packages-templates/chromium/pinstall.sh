...
if [ "`pwd`" != "/" ];then
 ...
 if [ ! -e usr/bin/chromium ];then #oe kirkstone
  echo '#!/bin/ash
exec /usr/lib/chromium/chromium-wrapper --test-type --no-sandbox --user-data-dir=/root/.config/chrome --disk-cache-size=10000000 --media-cache-size=10000000 $@' > usr/bin/chromium
