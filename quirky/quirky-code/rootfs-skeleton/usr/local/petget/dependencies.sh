...
if [ "$RUNNINGWOOF" = "no" ];then
  ...
  *-puppy-*)
  ...
  ;;
  *)
   #looking in a compat-distro db, then only puppy db allow is Packages-pet-${DISTRO_DB_SUBNAME}-*  121102
   DB_OTHERS="$(echo "$DB_OTHERS" | grep -v '\-puppy\-')"
   PUPDB="$(ls -1 ${PREPATH}Packages-pet-${DISTRO_DB_SUBNAME}-* 2>/dev/null)" #121102
