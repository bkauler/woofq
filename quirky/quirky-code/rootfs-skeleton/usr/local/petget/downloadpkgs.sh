...
#    fi
 ...
 #120907 scrollbar...
 export DEPS_DIALOG="<window title=\"$(gettext 'PETget Package Manager: download')\" icon-name=\"gtk-about\">
<vbox>
...
#${BADMSG3}"
 ...
 do
  ...
  if [ "`echo "$RETPARAMS" | grep 'RADIO_URL_LOCAL' | grep 'true'`" != "" ];then
   ...
   #rxvt -title "PETget Package Manager: download" -bg orange -fg black -geometry 80x10 -e wget ${DOWNLOADFROM}/${ONEFILE}
  ...
  if [ -f $DLPKG -a "$DLPKG" != "" ];then
   ...
   if [ $? -ne 0 ];then
    #xmessage -bg red -title "PETget Package Manager" "ERROR: faulty download of $DLPKG"
    export FAIL_DIALOG="<window title=\"$(gettext 'PETget Package Manager')\" icon-name=\"gtk-about\">
  <vbox>
  <pixmap><input file>/usr/local/lib/X11/pixmaps/error.xpm</input></pixmap>
   ...
   #xmessage -bg red -title "PETget Package Manager" "ERROR: Failed to download ${DLPKG}"
   export FAIL_DIALOG="<window title=\"$(gettext 'PETget Package Manager')\" icon-name=\"gtk-about\">
  <vbox>
  <pixmap><input file>/usr/local/lib/X11/pixmaps/error.xpm</input></pixmap>
...
if [ "$PASSEDPARAM" = "DOWNLOADONLY" ];then
 export DL_DIALOG="<window title=\"$(gettext 'PETget Package Manager')\" icon-name=\"gtk-about\">
  <vbox>
...
if [ -s /tmp/petget-installed-pkgs-log ];then
 ...
 export INSTALL_DIALOG="<window title=\"$(gettext 'PETget Package Manager')\" icon-name=\"gtk-about\">
  <vbox>
   <pixmap><input file>/usr/local/lib/X11/pixmaps/ok.xpm</input></pixmap>
...
" 
 ...
 if [ "$EXIT" = "BUTTON_TRIM_FAT" ];then
  ...
  export TRIM_DIALOG="<window title=\"$(gettext 'PETget Package Manager')\" icon-name=\"gtk-about\">
  <vbox>
