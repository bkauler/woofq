...
export PREVIEW_DIALOG="<window title=\"$(gettext 'PETget Package Manager: preinstall')\" icon-name=\"gtk-about\">
<vbox>
...
$(gettext 'Note: if the previous window indicated that there are missing dependencies, they were not found. Sometimes, a package database lists a dependency that does not actually exist anymore and is not required.')" 0 0
 ...
 if ! which gcc; then
  ...
  if [ "$NEEDGCC" ];then
   ...
   DEVXNAME="devx-${DISTRO_VERSION}-${DISTRO_FILE_PREFIX}.pet"
   echo "#!/bin/sh
sleep 3
...
$(gettext "This package has dependencies that are in the 'devx' PET file, which is Quirky's C/C++/Vala/Genie/BaCon mega-package, a complete compiling environment.")

$(gettext 'The devx file is named:') \Zb${DEVXNAME}\ZB

$(gettext "Please cancel installation, and install the devx PET first. You will find it in this repository:") \Zbpet-${DISTRO_DB_SUBNAME}\ZB\" 0 0" > /tmp/petget_devx_popup.sh #'geany
  ...
  do
   ...
   case $DEP_CNT in
    1)
     ...
     echo -n "Deps|" >> /tmp/petget_tabs #150808 shorten "Dependencies"
 ...
 export DEPS_DIALOG="<window title=\"$(gettext 'PETget Package Manager: dependencies')\" icon-name=\"gtk-about\">
<vbox>
